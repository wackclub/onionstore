...
CREATE TABLE "user" (
	...
	"country" varchar(2),
	"yswsDbFulfilled" boolean DEFAULT false NOT NULL
);
--> statement-breakpoint
...
--> statement-breakpoint
ALTER TABLE "payouts" ADD CONSTRAINT "payouts_userId_user_email_fk" FOREIGN KEY ("userId") REFERENCES "public"."user"("email") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
ALTER TABLE "shop_orders" ADD CONSTRAINT "shop_orders_userId_user_email_fk" FOREIGN KEY ("userId") REFERENCES "public"."user"("email") ON DELETE no action ON UPDATE no action;--> statement-breakpoint
...
CREATE VIEW "public"."users_with_tokens" AS (select "email", "displayName", "avatarUrl", "isAdmin", 
			GREATEST(
				COALESCE(
